...
public class GuiTodoList extends GuiContainer {
	...
	public GuiTodoList(Paper inv) {
		super(new ContainerTodoList(inv));
		this.paper = inv;
		this.xSize = 120;
		this.ySize = 180;
	...
	public void initGui() {
		...
		listName = new GuiTextField(-1, this.fontRendererObj, 20, 13, 90, 12);
		listName.setMaxStringLength(20);
		listName.setText(String.valueOf("Todo List"));
		...
		for (int i = 0; i < paper.entries.length; i++) {
			GuiTextField target = new GuiTextField(i, this.fontRendererObj, 14, 5 + (i * 10) + 26, 90, 10);
	...
	public void setListValues() {
		...
		paper.listName = (newListName == null || newListName.isEmpty()) ? "Todo List" : newListName;
		int pos = 0;
		for (GuiTextField field : targets) {
	...
	public void onGuiClosed() {
		setListValues();
		Bagels.network.sendToServer(new PacketToDoList(paper));
		super.onGuiClosed();
	}
